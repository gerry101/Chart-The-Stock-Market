 ...
 socket.on('chat message', function(msg) {
     ...
     $('#stockButtons').append('&nbsp;<div class="btn btn-warning" role="button">' + msg + ' <a class="stock" id="' + msg + '">X</a></div>');
     $('#symbolsearch').val('');
 });
 ...
 socket.on('message', function(message) {
     ...
     for (var i = 0; i < tempStocks.data.length; i++) {
         ...
         var tempdur = $.url().param('duration') || 3650;
         new Markit.InteractiveChartApi(tempsym, tempdur);
         $('#stockButtons').append('&nbsp;<div class="btn btn-warning" role="button">' + tempStocks.data[i] + ' <a class="stock" id="' + tempStocks.data[i] + '">X</a></div>');
         $('#symbolsearch').val('');
     }
 });
